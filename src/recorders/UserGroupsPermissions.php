...
class UserGroupsPermissions extends ConfigModelRecorder
{
    ...
    public function init(): void
    {
        ...
        foreach ($groups as $group) {
            \Craft::$app->projectConfig->onAdd(ProjectConfig::PATH_USER_GROUPS . '.' . $group->uid . '.permissions', function (Event $event) {
    ...
    public function onTriggered(ConfigEvent $event)
    {
        if (!$event->oldValue) {
            ...
            \Craft::$app->projectConfig->onAdd(ProjectConfig::PATH_USER_GROUPS . '.' . $event->tokenMatches[0] . '.permissions', function (Event $event) {
                Activity::getRecorder('userGroupsPermissions')->onUpdateGroupPerms($event);
            });
        }
